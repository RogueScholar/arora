...
public:
  ...
  bool call_acceptNavigationRequest(QWebFrame *frame,
                                    QNetworkRequest const &request,
                                    NavigationType type) {
    return SubWebPage::acceptNavigationRequest(frame, request, type);
  }

  QObject *call_createPlugin(QString const &classId, QUrl const &url,
                             QStringList const &paramNames,
                             QStringList const &paramValues) {
...
void tst_WebPage::webpage() {
  ...
  QCOMPARE(
      page.call_acceptNavigationRequest((QWebFrame *)0, QNetworkRequest(),
                                        QWebPage::NavigationTypeLinkClicked),
      true);
  QCOMPARE(
      page.call_createPlugin(QString(), QUrl(), QStringList(), QStringList()),
      (QObject *)0);
...
void tst_WebPage::acceptNavigationRequest_data() {
  ...
  QTest::newRow("null-noframe")
      << Qt::NoButton << Qt::NoModifier << false << QNetworkRequest()
      << QWebPage::NavigationTypeLinkClicked << true << 0;
  QTest::newRow("null-frame")
      << Qt::NoButton << Qt::NoModifier << true << QNetworkRequest()
      << QWebPage::NavigationTypeLinkClicked << true << 1;

  QTest::newRow("mailto-0")
      << Qt::NoButton << Qt::NoModifier << true
      << QNetworkRequest(QUrl("mailto:foo@bar.com"))
      << QWebPage::NavigationTypeLinkClicked << false << 0;
  QTest::newRow("mailto-1")
      << Qt::NoButton << Qt::NoModifier << false
      << QNetworkRequest(QUrl("mailto:foo@bar.com"))
      << QWebPage::NavigationTypeLinkClicked << false << 0;
  QTest::newRow("ftp-0") << Qt::NoButton << Qt::NoModifier << true
                         << QNetworkRequest(QUrl("ftp:foo@bar.com"))
                         << QWebPage::NavigationTypeLinkClicked << false << 0;
  QTest::newRow("ftp-1") << Qt::NoButton << Qt::NoModifier << false
                         << QNetworkRequest(QUrl("ftp:foo@bar.com"))
                         << QWebPage::NavigationTypeLinkClicked << false << 0;

  QTest::newRow("normal-0") << Qt::NoButton << Qt::NoModifier << false
                            << QNetworkRequest(QUrl("http://www.foo.com"))
                            << QWebPage::NavigationTypeLinkClicked << true << 0;
  QTest::newRow("normal-1") << Qt::NoButton << Qt::NoModifier << true
                            << QNetworkRequest(QUrl("http://www.foo.com"))
                            << QWebPage::NavigationTypeLinkClicked << true << 1;

  QTest::newRow("midclick-0")
      << Qt::MidButton << Qt::NoModifier << true
      << QNetworkRequest(QUrl("http://www.foo.com"))
      << QWebPage::NavigationTypeLinkClicked << false << 0;
  QTest::newRow("midclick-1")
      << Qt::MidButton << Qt::ShiftModifier << true
      << QNetworkRequest(QUrl("http://www.foo.com"))
      << QWebPage::NavigationTypeLinkClicked << false << 0;
  QTest::newRow("midclick-2")
      << Qt::MidButton << Qt::AltModifier << true
      << QNetworkRequest(QUrl("http://www.foo.com"))
      << QWebPage::NavigationTypeLinkClicked << false << 0;
}

// protected bool acceptNavigationRequest(QWebFrame *frame, QNetworkRequest
// const &request, NavigationType type)
void tst_WebPage::acceptNavigationRequest() {
  ...
  QCOMPARE(page.call_acceptNavigationRequest(frame, request, type),
...
// protected QObject *createPlugin(QString const &classId, QUrl const &url,
// QStringList const &paramNames, QStringList const &paramValues)
...
void tst_WebPage::handleUnsupportedContent() {
  ...
  page.mainFrame()->load(
      QUrl("http://exampletesttesttesttesttesttes.com/test.html"));
...
void tst_WebPage::linkedResources() {
  ...
  QString html =
      ...
      "href=\"../atom.xml\" title=\"Feed\" />"
      "<link rel=\"search\" type=\"application/opensearchdescription+xml\" "
      "href=\"http://external.foo/search.xml\" />"
      "</head>"
      "<body>"
  ...
  QCOMPARE(resources.at(0).href,
           QUrl("http://foobar.baz/foo/styles/common.css"));
  ...
  QCOMPARE(resources.at(3).type,
           QString("application/opensearchdescription+xml"));
  ...
  QCOMPARE(resources.at(0).href,
           QUrl("http://barbaz.foo/bar/styles/common.css"));
...
void tst_WebPage::javaScriptObjects_data() {
  ...
  QTest::newRow("qrc:/notfound.html")
      << QUrl("qrc:/notfound.html") << true << false;
  QTest::newRow("qrc:/startpage.html")
      << QUrl("qrc:/startpage.html") << true << true;
}
...
void tst_WebPage::javaScriptObjects() {
  ...
  QVariant windowExternalVariant =
      page.mainFrame()->evaluateJavaScript(QLatin1String("window.external"));
  QVariant windowAroraVariant =
      page.mainFrame()->evaluateJavaScript(QLatin1String("window.arora"));
