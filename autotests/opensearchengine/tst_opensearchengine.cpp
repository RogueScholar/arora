...
public:
  void call_imageChanged() { return SubOpenSearchEngine::imageChanged(); }

  void call_loadImage() const { return SubOpenSearchEngine::loadImage(); }

  QString call_parseTemplate(QString const &searchTerm,
                             QString const &searchTemplate) const {
...
public:
  SuggestionsTestNetworkReply(const QNetworkRequest &request,
                              QObject *parent = 0)
...
public:
  Delegate() : OpenSearchEngineDelegate(), callsCount(0) {}
...
void tst_OpenSearchEngine::image() {
  ...
  QString imageUrl =
      QString("data:image/png;base64,").append(imageBuffer.buffer().toBase64());
...
void tst_OpenSearchEngine::operatorequal_data() {
  ...
  QTest::newRow("null") << QString() << QString() << QString() << QString()
                        << QString() << Parameters() << Parameters() << true;
  QTest::newRow("name") << QString("x") << QString() << QString() << QString()
                        << QString() << Parameters() << Parameters() << false;
  QTest::newRow("description")
      << QString() << QString("x") << QString() << QString() << QString()
      << Parameters() << Parameters() << false;
  QTest::newRow("imageUrl")
      << QString() << QString() << QString("x") << QString() << QString()
      << Parameters() << Parameters() << false;
  QTest::newRow("parameters")
      << QString() << QString() << QString() << QString() << QString()
      << (Parameters() << Parameter("a", "b")) << Parameters() << false;
}
...
void tst_OpenSearchEngine::requestSuggestions_data() {
  ...
  QTest::newRow("foo") << QString("foo") << QString("get")
                       << QNetworkAccessManager::GetOperation;
  QTest::newRow("bar") << QString("bar") << QString("post")
                       << QNetworkAccessManager::PostOperation;
  QTest::newRow("baz") << QString("baz") << QString("put")
                       << QNetworkAccessManager::GetOperation;
}
...
void tst_OpenSearchEngine::requestSuggestions() {
  ...
  if (searchTerm.isEmpty()) {
    ...
    QCOMPARE(manager.lastOutgoingData,
             lastOperation == QNetworkAccessManager::PostOperation);
  }
}
...
void tst_OpenSearchEngine::searchParameters() {
  ...
  QCOMPARE(qvariant_cast<Parameters>(engine.property("searchParameters")),
           searchParameters);
...
void tst_OpenSearchEngine::searchUrl_data() {
  ...
  QTest::newRow("foo") << QString("foo")
                       << QString("http://foobar.baz/?q={searchTerms}")
                       << Parameters()
  ...
  QTest::newRow("parameters")
      << QString("baz") << QString("http://foobar.baz/?q={searchTerms}")
      << (Parameters() << Parameter("abc", "{searchTerms}")
                       << Parameter("x", "yz"))
...
void tst_OpenSearchEngine::parseTemplate_data() {
  ...
  QTest::newRow("allParameters")
      ...
      << QString("http://foobar.baz/?st=bar&amp;c=20&amp;si=0&amp;"
                 "sp=0&amp;l=%1&amp;ie=UTF-8&amp;oe=UTF-8")
             .arg(lang)
      << true;
  QTest::newRow("tricky")
      << QString("{count}")
      << QString("http://foobar.baz/q={searchTerms}&amp;count={count}")
      << QString("http://foobar.baz/q=%7Bcount%7D&amp;count=20") << true;
  QTest::newRow("multiple")
      << QString("abc")
      << QString("http://foobar.baz/?q={searchTerms}&amp;x={searchTerms}")
      << QString("http://foobar.baz/?q=abc&amp;x=abc") << true;
  QTest::newRow("referrer")
      << QString("foo")
      << QString("http://foobar.baz/"
                 "?q={searchTerms}&amp;a={source}&amp;b={ref:source}&amp;c={"
                 "referrer:source?}")
      << QString("http://foobar.baz/?q=foo&amp;a=tst_opensearchengine"
                 "&amp;b=tst_opensearchengine&amp;c=tst_opensearchengine")
      << true;
  QTest::newRow("inputEncoding")
      << QString("c++") << QString("http://foobar.baz/?q={searchTerms}")
      << QString("http://foobar.baz/?q=c%2B%2B") << true;
}

// protected QString parseTemplate(QString const &searchTerm, QString const
// &searchTemplate) const
...
void tst_OpenSearchEngine::languageCodes_data() {
  ...
  QTest::newRow("pt_BR") << QString("pt_BR")
                         << QString("http://foobar.baz/?l=pt-BR");
}
...
void tst_OpenSearchEngine::languageCodes() {
  ...
  SubOpenSearchEngine engine;
  QCOMPARE(engine.call_parseTemplate(
               QString("foo"), QString("http://foobar.baz/?l={language}")),
           url);
}
...
void tst_OpenSearchEngine::delegate() {
  ...
  QNetworkRequest request(QUrl(
      engine.call_parseTemplate(QString("baz"), engine.searchUrlTemplate())));
  ...
  QUrl url(
      engine.call_parseTemplate(QString("baz"), engine.searchUrlTemplate()));
  QCOMPARE(delegate.lastRequest.url().toString(QUrl::RemoveQuery),
  ...
  QCOMPARE(query, QStringList() << "a=b"
                                << "b=c");
}
