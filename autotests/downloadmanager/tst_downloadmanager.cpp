...
public:
  SubDownloadManager(QWidget *parent = 0) : DownloadManager(parent) {}
};
...
void tst_DownloadManager::download_data() {
  ...
  QTest::newRow("badfile") << (QStringList() << BIGFILE ".dne")
                           << (QStringList() << BIGFILENAME ".dne") << 1
                           << false;
  QTest::newRow("twofiles")
      << (QStringList() << BIGFILE << BIGFILE)
      << (QStringList() << BIGFILENAME << BIGFILENAME2) << 2 << true;
}
...
void tst_DownloadManager::removePolicy() {
  QFETCH(DownloadManager::RemovePolicy, removePolicy);
  {
    ...
    QCOMPARE(view->model()->rowCount(),
             (removePolicy == DownloadManager::SuccessFullDownload) ? 0 : 1);
  }
  ...
  QVERIFY(view);
  QCOMPARE(view->model()->rowCount(),
           removePolicy == DownloadManager::Never ? 1 : 0);
}
