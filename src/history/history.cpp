...
HistoryModel::HistoryModel(HistoryManager *history, QObject *parent)
  ...
  Q_ASSERT(m_history);
  connect(m_history, SIGNAL(historyGoingToChange()), this,
          SLOT(historyGoingToChange()));
  connect(m_history, SIGNAL(historyReset()), this, SLOT(historyReset()));
  connect(m_history, SIGNAL(entryRemoved(const HistoryEntry &)), this,
          SLOT(historyReset()));

  connect(m_history, SIGNAL(entryAdded(const HistoryEntry &)), this,
          SLOT(entryAdded()));
  connect(m_history, SIGNAL(entryUpdated(int)), this, SLOT(entryUpdated(int)));
}

void HistoryModel::historyReset() { endResetModel(); }

void HistoryModel::historyGoingToChange() { beginResetModel(); }
...
*/
HistoryMenuModel::HistoryMenuModel(HistoryTreeModel *sourceModel,
                                   QObject *parent)
    : QAbstractProxyModel(parent), m_treeModel(sourceModel) {
...
QModelIndex
HistoryMenuModel::mapFromSource(const QModelIndex &sourceIndex) const {
...
QModelIndex HistoryMenuModel::mapToSource(const QModelIndex &proxyIndex) const {
  ...
  if (proxyIndex.internalId() == -1) {
    ...
    if (bumpedItems <= MOVEDROWS &&
        bumpedItems == sourceModel()->rowCount(m_treeModel->index(0, 0)))
      --bumpedItems;
    return m_treeModel->index(proxyIndex.row() - bumpedItems,
                              proxyIndex.column());
  }

  QModelIndex historyIndex = m_treeModel->sourceModel()->index(
      proxyIndex.internalId(), proxyIndex.column());
  QModelIndex treeIndex = m_treeModel->mapFromSource(historyIndex);
  return treeIndex;
}

QModelIndex HistoryMenuModel::index(int row, int column,
  ...
  QModelIndex treeIndex =
      m_treeModel->index(row + bumpedItems, column, treeIndexParent);
...
QModelIndex HistoryMenuModel::parent(const QModelIndex &index) const {
  ...
  QModelIndex historyIndex =
      m_treeModel->sourceModel()->index(index.internalId(), 0);
  QModelIndex treeIndex = m_treeModel->mapFromSource(historyIndex);
  ...
  if (bumpedItems <= MOVEDROWS &&
      bumpedItems == sourceModel()->rowCount(sourceModel()->index(0, 0)))
    --bumpedItems;
  return createIndex(bumpedItems + treeIndexParent.row(),
                     treeIndexParent.column(), sr);
}
...
HistoryMenu::HistoryMenu(QWidget *parent)
    : ModelMenu(parent), m_history(0), m_historyMenuModel(0) {
  setMaxRows(7);
...
bool HistoryMenu::prePopulated() {
  if (!m_history) {
    ...
    m_historyMenuModel =
        new HistoryMenuModel(m_history->historyTreeModel(), this);
...
void HistoryMenu::showHistoryDialog() {
  ...
  connect(dialog, SIGNAL(openUrl(const QUrl &, const QString &)), this,
...
void HistoryMenu::clearHistoryDialog() {
  if (m_history &&
      QMessageBox::question(0, tr("Clear History"),
                            tr("Do you want to clear the history?"),
...
HistoryDialog::HistoryDialog(QWidget *parent, HistoryManager *setHistory)
  ...
  connect(search, SIGNAL(textChanged(QString)), proxyModel,
  ...
  connect(tree, SIGNAL(activated(const QModelIndex &)), this, SLOT(open()));
...
void HistoryDialog::open() {
  ...
  BrowserApplication::instance()->setEventKeyboardModifiers(
...
HistoryFilterModel::HistoryFilterModel(QAbstractItemModel *sourceModel,
                                       QObject *parent)
    : QAbstractProxyModel(parent), m_loaded(false) {
  m_frecencyTimer.setSingleShot(true);
  connect(&m_frecencyTimer, SIGNAL(timeout()), this, SLOT(refreshFrecencies()));
...
void HistoryFilterModel::startFrecencyTimer() {
  // schedule us to recalculate the frecencies once per day, at 3:00 am (aka
  // 03h00)
...
void HistoryFilterModel::sourceDataChanged(const QModelIndex &topLeft,
                                           const QModelIndex &bottomRight) {
  emit dataChanged(mapFromSource(topLeft), mapFromSource(bottomRight));
}

QVariant HistoryFilterModel::headerData(int section,
                                        Qt::Orientation orientation,
                                        int role) const {
  return sourceModel()->headerData(section, orientation, role);
}

void HistoryFilterModel::recalculateFrecencies() { sourceReset(); }
...
QModelIndex
HistoryFilterModel::mapToSource(const QModelIndex &proxyIndex) const {
  load();
...
QModelIndex
HistoryFilterModel::mapFromSource(const QModelIndex &sourceIndex) const {
  load();
  ...
  QList<HistoryData>::iterator pos =
      qBinaryFind(m_filteredRows.begin(), m_filteredRows.end(),
                  HistoryData(sourceOffset, -1));

  if (pos == m_filteredRows.end())
    return QModelIndex();

  return createIndex(pos - m_filteredRows.begin(), sourceIndex.column(),
                     sourceOffset);
}

QModelIndex HistoryFilterModel::index(int row, int column,
                                      const QModelIndex &parent) const {
  load();
...
void HistoryFilterModel::load() const {
  ...
  for (int i = 0; i < sourceModel()->rowCount(); ++i) {
    ...
    if (!m_historyHash.contains(url)) {
      ...
      QList<HistoryData>::iterator pos =
          qBinaryFind(m_filteredRows.begin(), m_filteredRows.end(),
                      HistoryData(m_historyHash[url], -1));
...
void HistoryFilterModel::sourceRowsInserted(const QModelIndex &parent,
                                            int start, int end) {
  ...
  m_filteredRows.insert(0, HistoryData(sourceModel()->rowCount(),
                                       frecencyScore(idx) + currentFrecency));
  m_historyHash.insert(url, sourceModel()->rowCount());
  endInsertRows();
}

void HistoryFilterModel::sourceRowsRemoved(const QModelIndex &, int start,
                                           int end) {
  Q_UNUSED(start);
  Q_UNUSED(end);
  sourceReset();
...
/*
    ...
    the users intention.
*/
bool HistoryFilterModel::removeRows(int row, int count,
...
int HistoryFilterModel::frecencyScore(const QModelIndex &sourceIndex) const {
  QDateTime loadTime =
      sourceModel()->data(sourceIndex, HistoryModel::DateTimeRole).toDateTime();
...
HistoryTreeModel::HistoryTreeModel(QAbstractItemModel *sourceModel,
                                   QObject *parent)
    : QAbstractProxyModel(parent), removingDown(false) {
  setSourceModel(sourceModel);
}

QVariant HistoryTreeModel::headerData(int section, Qt::Orientation orientation,
                                      int role) const {
  return sourceModel()->headerData(section, orientation, role);
}
...
int HistoryTreeModel::rowCount(const QModelIndex &parent) const {
  ...
  if (!parent.isValid()) {
    ...
    for (int i = 0; i < totalRows; ++i) {
      QDate rowDate =
          sourceModel()->index(i, 0).data(HistoryModel::DateRole).toDate();
...
QModelIndex HistoryTreeModel::mapToSource(const QModelIndex &proxyIndex) const {
  ...
  return sourceModel()->index(startDateRow + proxyIndex.row(),
                              proxyIndex.column());
}

QModelIndex HistoryTreeModel::index(int row, int column,
                                    const QModelIndex &parent) const {
  if (row < 0 || column < 0 || column >= columnCount(parent) ||
      parent.column() > 0)
    return QModelIndex();
...
void HistoryTreeModel::sourceRowsInserted(const QModelIndex &parent, int start,
                                          int end) {
...
QModelIndex
HistoryTreeModel::mapFromSource(const QModelIndex &sourceIndex) const {
  ...
  it = qLowerBound(m_sourceRowCache.begin(), m_sourceRowCache.end(),
                   sourceIndex.row());
...
bool HistoryTreeModel::removeRows(int row, int count,
                                  const QModelIndex &parent) {
  if (row < 0 || count <= 0 || row + count > rowCount(parent))
    return false;
...
void HistoryTreeModel::sourceRowsRemoved(const QModelIndex &parent, int start,
                                         int end) {
  if (!removingDown) {
    beginResetModel();
    m_sourceRowCache.clear();
