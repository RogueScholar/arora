...
OpenSearchManager::OpenSearchManager(QObject *parent)
    : QObject(parent), m_autoSaver(new AutoSaver(this)) {
  connect(this, SIGNAL(changed()), m_autoSaver, SLOT(changeOccurred()));
...
QString OpenSearchManager::currentEngineName() const { return m_current; }
...
int OpenSearchManager::enginesCount() const { return m_engines.count(); }
...
void OpenSearchManager::addEngine(const QUrl &url) {
  ...
  QNetworkReply *reply =
      BrowserApplication::networkAccessManager()->get(QNetworkRequest(url));
...
void OpenSearchManager::removeEngine(const QString &name) {
  ...
  QString file =
      QDir(enginesDirectory()).filePath(generateEngineFileName(name));
  QFile::remove(file);
...
QString
OpenSearchManager::generateEngineFileName(const QString &engineName) const {
  QString fileName;
...
void OpenSearchManager::save() {
  ...
  QHash<QString, OpenSearchEngine *>::const_iterator i =
      m_keywords.constBegin();
  QHash<QString, OpenSearchEngine *>::const_iterator end =
      m_keywords.constEnd();
  int j = 0;
  for (; i != end; ++i) {
...
void OpenSearchManager::load() {
  ...
  QSettings settings;
  settings.beginGroup(QLatin1String("openSearch"));
  m_current = settings.value(QLatin1String("engine"), QLatin1String("Google"))
                  .toString();
...
bool OpenSearchManager::confirmAddition(OpenSearchEngine *engine) {
  ...
  QMessageBox::StandardButton button = QMessageBox::question(
      0, QString(),
      tr("Do you want to add the following engine to your list of search "
         "engines?<br /><br />"
         "Name: %1<br />Searches on: %2")
          .arg(engine->name(), host),
...
OpenSearchEngine *
OpenSearchManager::engineForKeyword(const QString &keyword) const {
  if (keyword.isEmpty())
    return 0;
...
void OpenSearchManager::setEngineForKeyword(const QString &keyword,
...
QStringList
OpenSearchManager::keywordsForEngine(OpenSearchEngine *engine) const {
  return m_keywords.keys(engine);
}

void OpenSearchManager::setKeywordsForEngine(OpenSearchEngine *engine,
