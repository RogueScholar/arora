...
CookieJar::CookieJar(QObject *parent)
    : NetworkCookieJar(parent), m_loaded(false),
      m_saveTimer(new AutoSaver(this)), m_filterTrackingCookies(false),
      m_acceptCookies(AcceptOnlyFromSitesNavigatedTo), m_isPrivate(false) {}
...
void CookieJar::setPrivate(bool isPrivate) { m_isPrivate = isPrivate; }
...
void CookieJar::load() {
  ...
  QSettings cookieSettings(
      BrowserApplication::dataFilePath(QLatin1String("cookies.ini")),
      QSettings::IniFormat);
  if (!m_isPrivate) {
    setAllCookies(qvariant_cast<QList<QNetworkCookie>>(
        cookieSettings.value(QLatin1String("cookies"))));
  }
  cookieSettings.beginGroup(QLatin1String("Exceptions"));
  m_exceptions_block =
      cookieSettings.value(QLatin1String("block")).toStringList();
  m_exceptions_allow =
      cookieSettings.value(QLatin1String("allow")).toStringList();
  m_exceptions_allowForSession =
      cookieSettings.value(QLatin1String("allowForSession")).toStringList();
  qSort(m_exceptions_block.begin(), m_exceptions_block.end());
  qSort(m_exceptions_allow.begin(), m_exceptions_allow.end());
  qSort(m_exceptions_allowForSession.begin(),
        m_exceptions_allowForSession.end());
...
void CookieJar::loadSettings() {
  ...
  QByteArray value = settings
                         ...
                         .toByteArray();
  QMetaEnum acceptPolicyEnum = staticMetaObject.enumerator(
      staticMetaObject.indexOfEnumerator("AcceptPolicy"));
  m_acceptCookies =
  ...
  value = settings
              .value(QLatin1String("keepCookiesUntil"),
                     QLatin1String("KeepUntilExpire"))
              .toByteArray();
  QMetaEnum keepPolicyEnum = staticMetaObject.enumerator(
      staticMetaObject.indexOfEnumerator("KeepPolicy"));
  m_keepCookies =
  ...
  m_loaded = true;
  m_filterTrackingCookies = settings
                                .value(QLatin1String("filterTrackingCookies"),
                                       m_filterTrackingCookies)
                                .toBool();
...
void CookieJar::save() {
  ...
  cookieSettings.setValue(QLatin1String("cookies"),
                          qVariantFromValue<QList<QNetworkCookie>>(cookies));
  cookieSettings.beginGroup(QLatin1String("Exceptions"));
  ...
  cookieSettings.setValue(QLatin1String("allowForSession"),
                          m_exceptions_allowForSession);
  ...
  QMetaEnum acceptPolicyEnum = staticMetaObject.enumerator(
      staticMetaObject.indexOfEnumerator("AcceptPolicy"));
  settings.setValue(
      QLatin1String("acceptCookies"),
      QLatin1String(acceptPolicyEnum.valueToKey(m_acceptCookies)));

  QMetaEnum keepPolicyEnum = staticMetaObject.enumerator(
      staticMetaObject.indexOfEnumerator("KeepPolicy"));
  settings.setValue(QLatin1String("keepCookiesUntil"),
                    QLatin1String(keepPolicyEnum.valueToKey(m_keepCookies)));
...
bool CookieJar::setCookiesFromUrl(const QList<QNetworkCookie> &cookieList,
  ...
  bool eAllowSession =
      !eBlock && !eAllow && isOnDomainList(m_exceptions_allowForSession, host);
  ...
  if ((acceptInitially && !eBlock) ||
    ...
    foreach (QNetworkCookie cookie, cookieList) {
      ...
      if (!(m_filterTrackingCookies && cookie.name().startsWith("__utm"))) {
        ...
        if (m_keepCookies == KeepUntilTimeLimit && !cookie.isSessionCookie() &&
...
bool CookieJar::isOnDomainList(const QStringList &rules,
  ...
  foreach (const QString &rule, rules) {
    if (rule.startsWith(QLatin1String("."))) {
      ...
      if (!domainEnding.isEmpty() && domainEnding.at(0) == QLatin1Char('.') &&
