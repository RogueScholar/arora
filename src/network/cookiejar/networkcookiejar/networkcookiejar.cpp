...
NetworkCookieJar::~NetworkCookieJar() { delete d; }
...
inline static bool shorterPaths(const QNetworkCookie &c1,
                                const QNetworkCookie &c2) {
...
QList<QNetworkCookie> NetworkCookieJar::cookiesForUrl(const QUrl &url) const {
  ...
  for (; i != cookies.end();) {
...
#if defined(NETWORKCOOKIEJAR_DEBUG)
      qDebug() << __FUNCTION__ << "Ignoring cookie, path does not match" << *i
               << urlPath;
#endif
      i = cookies.erase(i);
      continue;
    ...
    if (!isSecure && i->isSecure()) {
      i = cookies.erase(i);
#if defined(NETWORKCOOKIEJAR_DEBUG)
      qDebug() << __FUNCTION__ << "Ignoring cookie, security mismatch" << *i
               << !isSecure;
#endif
      continue;
    }
...
#if defined(NETWORKCOOKIEJAR_DEBUG)
      qDebug() << __FUNCTION__ << "Ignoring cookie, expiration issue" << *i
               << now;
#endif
      i = cookies.erase(i);
      continue;
...
#if defined(NETWORKCOOKIEJAR_DEBUG)
  qDebug() << "NetworkCookieJar::" << __FUNCTION__ << "returning"
           << cookies.count();
  qDebug() << cookies;
#endif
  return cookies;
...
bool NetworkCookieJar::setCookiesFromUrl(
  ...
  QString defaultPath =
      fullUrlPath.mid(0, fullUrlPath.lastIndexOf(QLatin1Char('/')) + 1);
  ...
  foreach (QNetworkCookie cookie, cookieList) {
    ...
    bool alreadyDead =
        !cookie.isSessionCookie() && cookie.expirationDate() < now;

    if (cookie.path().isEmpty()) {
      cookie.setPath(defaultPath);
    }
    // Matching the behavior of Firefox, no path checking is done when setting
    // cookies Safari does something even odder, when that paths don't match it
    // keeps the cookie, but changes the paths to the default path
#if 0
...
#ifdef NETWORKCOOKIEJAR_LOGREJECTEDCOOKIES
      qDebug() << "NetworkCookieJar::" << __FUNCTION__
               << "Blocked cookie because: domain doesn't match: " << cookie
               << url;
#endif
      continue;
    }
    ...
    for (; it != cookies.constEnd(); ++it) {
      if (cookie.name() == it->name() && cookie.domain() == it->domain() &&
          cookie.path() == it->path()) {
...
bool NetworkCookieJarPrivate::matchingPath(const QNetworkCookie &cookie,
...
bool NetworkCookieJarPrivate::matchesBlacklist(const QString &string) const {
  ...
  QStringList::const_iterator i = qBinaryFind(
      secondLevelDomains.constBegin(), secondLevelDomains.constEnd(), string);
  return (i != secondLevelDomains.constEnd());
}

bool NetworkCookieJarPrivate::matchingDomain(const QNetworkCookie &cookie,
  ...
  QStringList urlParts =
      url.host().toLower().split(QLatin1Char('.'), QString::SkipEmptyParts);
  if (urlParts.isEmpty())
    return false;
...
void NetworkCookieJar::setSecondLevelDomains(
    const QStringList &secondLevelDomains) {
