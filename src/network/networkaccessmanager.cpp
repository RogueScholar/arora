...
NetworkAccessManager::NetworkAccessManager(QObject *parent)
    : NetworkAccessManagerProxy(parent), m_adblockNetwork(0) {
  connect(this,
...
void NetworkAccessManager::loadSettings() {
  ...
  QList<QSslCertificate> ca_new = QSslCertificate::fromData(
      settings.value(QLatin1String("CaCertificates")).toByteArray());
  ca_list += ca_new;
  ...
  QStringList acceptList = settings
                               .value(QLatin1String("acceptLanguages"),
                                      AcceptLanguageDialog::defaultAcceptList())
                               .toStringList();
  m_acceptLanguage = AcceptLanguageDialog::httpString(acceptList);

  bool cacheEnabled =
      settings.value(QLatin1String("cacheEnabled"), true).toBool();
...
void NetworkAccessManager::authenticationRequired(QNetworkReply *reply,
                                                  QAuthenticator *auth) {
  ...
  passwordDialog.iconLabel->setPixmap(
      mainWindow->style()
          ->standardIcon(QStyle::SP_MessageBoxQuestion, 0, mainWindow)
          .pixmap(32, 32));

  QString introMessage =
      tr("<qt>Enter username and password for \"%1\" at %2</qt>");
  introMessage =
      introMessage.arg(QString(auth->realm())
                           .toHtmlEscaped()
...
void NetworkAccessManager::proxyAuthenticationRequired(
...
QString NetworkAccessManager::certToFormattedString(QSslCertificate cert) {
  ...
  QMultiMap<QSsl::AlternativeNameEntryType, QString> names =
  ...
  if (names.count() > 0) {
    ...
    list += QStringList(names.values(QSsl::DnsEntry))
                .join(QLatin1String("</li><li>"));
    list += QLatin1String("</li></ul>");
    message << list;
  }

  QString result = QLatin1String("<p>") +
                   message.join(QLatin1String("<br />")) +
                   QLatin1String("</p>");

  return result;
}

void NetworkAccessManager::sslErrors(QNetworkReply *reply,
  ...
  QSettings settings;
  QList<QSslCertificate> ca_merge = QSslCertificate::fromData(
      settings.value(QLatin1String("CaCertificates")).toByteArray());
  ...
  int ret = QMessageBox::warning(
      mainWindow, QCoreApplication::applicationName() + tr(" - SSL Errors"),
      tr("<qt>SSL Errors:"
         "<br/><br/>for: <tt>%1</tt>"
         "<ul><li>%2</li></ul>\n\n"
         "Do you want to ignore these errors?</qt>")
  ...
  if (ret == QMessageBox::Yes) {
    if (ca_new.count() > 0) {
      ...
      ret = QMessageBox::question(
          mainWindow, QCoreApplication::applicationName(),
...
NetworkAccessManager::createRequest(QNetworkAccessManager::Operation op,
  ...
  if (m_schemeHandlers.contains(request.url().scheme()))
    reply = m_schemeHandlers[request.url().scheme()]->createRequest(
