...
int TabShortcut::tab() { return m_tab; }

TabBar::TabBar(QWidget *parent)
    : QTabBar(parent), m_viewTabBarAction(0), m_showTabBarWhenOneTab(true) {
  ...
  connect(m_viewTabBarAction, SIGNAL(triggered()), this, SLOT(viewTabBar()));

  setMovable(true);
}

bool TabBar::showTabBarWhenOneTab() const { return m_showTabBarWhenOneTab; }
...
QAction *TabBar::viewTabBarAction() const { return m_viewTabBarAction; }

QTabBar::ButtonPosition TabBar::freeSide() {
  QTabBar::ButtonPosition side = (QTabBar::ButtonPosition)style()->styleHint(
      QStyle::SH_TabBar_CloseButtonPosition, 0, this);
...
void TabBar::contextMenuRequested(const QPoint &position) {
  ...
  if (-1 != index) {
    QAction *action =
        menu.addAction(tr("Duplicate Tab"), this, SLOT(cloneTab()));
    action->setData(index);

    menu.addSeparator();

    action = menu.addAction(tr("&Close Tab"), this, SLOT(closeTab()),
                            QKeySequence::Close);
    action->setData(index);
    ...
    action = menu.addAction(tr("Reload Tab"), this, SLOT(reloadTab()),
...
QSize TabBar::tabSizeHint(int index) const {
  ...
  return sizeHint.boundedTo(
      QSize(fm.width(QLatin1Char('M')) * 18, sizeHint.height()));
}
