...
WebView::WebView(QWidget *parent)
    : QWebView(parent), m_progress(0), m_currentZoom(100),
      m_page(new WebPage(this))
#if QT_VERSION >= 0x040600 || defined(WEBKIT_TRUNK)
      ,
      m_enableAccessKeys(true), m_accessKeysPressed(false)
#endif
{
  setPage(m_page);
  ...
  connect(page(), SIGNAL(aboutToLoadUrl(const QUrl &)), this,
  ...
  connect(m_page, SIGNAL(scrollRequested(int, int, const QRect &)), this,
          SLOT(hideAccessKeys()));
#endif
  loadSettings();
}
...
void WebView::loadSettings() {
  ...
  m_enableAccessKeys =
      settings.value(QLatin1String("enableAccessKeys"), m_enableAccessKeys)
          .toBool();
...
QUrl WebView::guessUrlFromString(const QString &string) {
  ...
  // If the string is missing the scheme or the scheme is not valid prepend a
  // scheme
  QString scheme = url.scheme();
  if (scheme.isEmpty() || scheme.contains(QLatin1Char('.')) ||
      scheme == QLatin1String("localhost")) {
    // Do not do anything for strings such as "foo", only "foo.com"
    int dotIndex = trimmedString.indexOf(QLatin1Char('.'));
    if (dotIndex != -1 ||
        trimmedString.startsWith(QLatin1String("localhost"))) {
      const QString hostscheme = trimmedString.left(dotIndex).toLower();
      QByteArray scheme = (hostscheme == QLatin1String("ftp")) ? "ftp" : "http";
      trimmedString =
          QLatin1String(scheme) + QLatin1String("://") + trimmedString;
    }
...
void WebView::contextMenuEvent(QContextMenuEvent *event) {
  ...
  if (!r.linkUrl().isEmpty()) {
    QAction *newWindowAction = menu->addAction(
        tr("Open in New &Window"), this, SLOT(openActionUrlInNewWindow()));
    newWindowAction->setData(r.linkUrl());
    QAction *newTabAction = menu->addAction(tr("Open in New &Tab"), this,
                                            SLOT(openActionUrlInNewTab()));
    newTabAction->setData(r.linkUrl());
    menu->addSeparator();
    menu->addAction(tr("Save Lin&k"), this, SLOT(downloadLinkToDisk()));
    menu->addAction(tr("&Bookmark This Link"), this, SLOT(bookmarkLink()))
        ->setData(r.linkUrl());
    menu->addSeparator();
    if (!page()->selectedText().isEmpty())
      menu->addAction(pageAction(QWebPage::Copy));
    menu->addAction(tr("&Copy Link Location"), this,
                    SLOT(copyLinkToClipboard()));
  }
  ...
  if (!r.imageUrl().isEmpty()) {
    ...
    QAction *newWindowAction =
        menu->addAction(tr("Open Image in New &Window"), this,
                        SLOT(openActionUrlInNewWindow()));
    newWindowAction->setData(r.imageUrl());
    QAction *newTabAction = menu->addAction(tr("Open Image in New &Tab"), this,
                                            SLOT(openActionUrlInNewTab()));
    ...
    menu->addAction(tr("C&opy Image Location"), this,
                    SLOT(copyImageLocationToClipboard()))
        ->setData(r.imageUrl().toString());
    menu->addSeparator();
    menu->addAction(tr("Block Image"), this, SLOT(blockImage()))
        ->setData(r.imageUrl().toString());
  }
  ...
  if (!page()->selectedText().isEmpty()) {
    ...
    for (int i = 0; i < list.count(); ++i) {
      ...
      OpenSearchEngine *engine =
          ToolbarSearch::openSearchManager()->engine(name);
      QAction *action = new OpenSearchEngineAction(engine, searchMenu);
      searchMenu->addAction(action);
      action->setData(name);
    }

    connect(searchMenu, SIGNAL(triggered(QAction *)), this,
            SLOT(searchRequested(QAction *)));
  }
  ...
  if (!element.isNull() &&
      ...
      element.attribute(QLatin1String("type"), QLatin1String("text")) ==
          QLatin1String("text")) {
    if (menu->isEmpty()) {
      menu->addAction(pageAction(QWebPage::Copy));
    } else {
    ...
    menu->addAction(tr("Add to the toolbar search"), this,
                    SLOT(addSearchEngine()))
        ->setData(variant);
  }
#endif
...
void WebView::openActionUrlInNewTab() {
  if (QAction *action = qobject_cast<QAction *>(sender())) {
    QWebPage *page =
        tabWidget()->getView(TabWidget::NewNotSelectedTab, this)->page();
...
void WebView::searchRequested(QAction *action) {
  ...
  if (index.canConvert<QString>()) {
    OpenSearchEngine *engine =
        ToolbarSearch::openSearchManager()->engine(index.toString());
...
void WebView::addSearchEngine() {
  ...
  if (formElement.isNull() ||
      formElement.attribute(QLatin1String("action")).isEmpty())
    return;

  QString method =
      formElement.attribute(QLatin1String("method"), QLatin1String("get"))
          .toLower();
  if (method != QLatin1String("get")) {
    QMessageBox::warning(
        this, tr("Method not supported"),
        tr("%1 method is not supported.").arg(method.toUpper()));
    return;
  }

  QUrl searchUrl(page()->mainFrame()->baseUrl().resolved(
      QUrl(formElement.attribute(QLatin1String("action")))));
  ...
  foreach (QWebElement inputField, inputFields) {
    QString type =
        inputField.attribute(QLatin1String("type"), QLatin1String("text"));
    QString name = inputField.attribute(QLatin1String("name"));
    QString value =
        inputField.evaluateJavaScript(QLatin1String("this.value")).toString();
    ...
    if (type == QLatin1String("submit")) {
      ...
      if (inputField.evaluateJavaScript(QLatin1String("this.checked"))
              .toBool()) {
        query.addQueryItem(name, value);
      }
  ...
  foreach (QWebElement selectField, selectFields) {
    ...
    int selectedIndex =
        selectField.evaluateJavaScript(QLatin1String("this.selectedIndex"))
            .toInt();
    if (selectedIndex == -1)
      continue;
  ...
  if (searchEngines.count() > 1) {
    QString searchEngine = QInputDialog::getItem(
        this, tr("Search engine"), tr("Choose the desired search engine"),
        searchEngines.keys(), 0, false, &ok);
    if (!ok)
      return;
  ...
  QString engineName;
  QWebElementCollection labels = formElement.findAll(
      QString(QLatin1String("label[for=\"%1\"]")).arg(elementName));
  if (labels.count() > 0)
    engineName = labels.at(0).toPlainText();

  engineName = QInputDialog::getText(this, tr("Engine name"),
                                     tr("Type in a name for the engine"),
...
void WebView::setProgress(int progress) { m_progress = progress; }
...
void WebView::loadUrl(const QUrl &url, const QString &title) {
  if (url.scheme() == QLatin1String("javascript")) {
    QString scriptSource = QUrl::fromPercentEncoding(
        url.toString(Q_FLAGS(QUrl::TolerantMode | QUrl::RemoveScheme))
            .toLatin1());
...
QString WebView::lastStatusBarText() const { return m_statusBarText; }
...
void WebView::accessKeyShortcut() {
  if (!hasFocus() || !m_accessKeysPressed || !m_enableAccessKeys)
    return;
...
void WebView::showAccessKeys() {
  ...
  supportedElement << QLatin1String("input") << QLatin1String("a")
                   << QLatin1String("area") << QLatin1String("button")
                   << QLatin1String("label") << QLatin1String("legend")
  ...
  QRect viewport =
      QRect(m_page->mainFrame()->scrollPosition(), m_page->viewportSize());
  ...
  foreach (const QString &elementType, supportedElement) {
    ...
    foreach (const QWebElement &element, result) {
      ...
      QString accessKeyAttribute =
          element.attribute(QLatin1String("accesskey")).toUpper();
  ...
  foreach (const QString &elementType, supportedElement) {
    ...
    foreach (const QWebElement &element, result) {
      const QRect geometry = element.geometry();
      if (unusedKeys.isEmpty() || alreadyLabeled.contains(element) ||
          geometry.size().isEmpty() || !viewport.contains(geometry.topLeft())) {
        continue;
      }
      QChar accessKey;
...
void WebView::makeAccessKeyLabel(const QChar &accessKey,
