...
ModelToolBar::ModelToolBar(QWidget *parent) : QToolBar(parent), m_model(0) {
  if (isVisible())
    build();
...
ModelToolBar::ModelToolBar(const QString &title, QWidget *parent)
    : QToolBar(title, parent), m_model(0) {
  if (isVisible())
    build();
...
void ModelToolBar::setModel(QAbstractItemModel *model) {
  if (m_model) {
    disconnect(m_model, SIGNAL(modelReset()), this, SLOT(build()));
  ...
  if (m_model) {
    connect(m_model, SIGNAL(modelReset()), this, SLOT(build()));
    connect(m_model, SIGNAL(rowsInserted(const QModelIndex &, int, int)), this,
            SLOT(build()));
    connect(m_model, SIGNAL(rowsRemoved(const QModelIndex &, int, int)), this,
            SLOT(build()));
    connect(m_model,
...
QAbstractItemModel *ModelToolBar::model() const { return m_model; }

void ModelToolBar::setRootIndex(const QModelIndex &index) {
  m_rootIndex = index;
}

QModelIndex ModelToolBar::rootIndex() const { return m_rootIndex; }
...
ModelMenu *ModelToolBar::createMenu() { return new ModelMenu(this); }
