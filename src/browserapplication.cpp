...
BrowserApplication::BrowserApplication(int &argc, char **argv)
...
#ifndef AUTOTESTS
  connect(this, SIGNAL(messageReceived(QLocalSocket *)), this,
          SLOT(messageReceived(QLocalSocket *)));
  ...
  QWebSettings::globalSettings()->setFontSize(
      QWebSettings::DefaultFixedFontSize, 16);
...
void BrowserApplication::messageReceived(QLocalSocket *socket) {
  ...
  if (!message.startsWith(QLatin1String("aroramessage://"))) {
    ...
    if (QUrl(message) == m_lastAskedUrl &&
      ...
      qWarning() << "Possible recursive openUrl called, ignoring url:"
                 << m_lastAskedUrl;
      return;
    }
...
#ifdef Q_OS_WIN
    QString winid =
        QString(QLatin1String("%1")).arg((qlonglong)mainWindow()->winId());
#else
    mainWindow()->show();
    mainWindow()->setFocus();
    mainWindow()->raise();
...
#endif
#ifdef BROWSERAPPLICATION_DEBUG
    qDebug() << "BrowserApplication::" << __FUNCTION__ << "sending win id"
...
#ifdef BROWSERAPPLICATION_DEBUG
    qDebug() << "BrowserApplication::" << __FUNCTION__
             << "got win id:" << winid;
#endif
#ifdef Q_OS_WIN
...
void BrowserApplication::quitBrowser() {
  ...
  if (QSettings()
          .value(QLatin1String("tabs/confirmClosingMultipleTabs"), true)
          .toBool()) {
    clean();
    int tabCount = 0;
...
void BrowserApplication::postLaunch() {
  ...
  if (directory.isEmpty())
    directory = QDir::homePath() + QLatin1String("/.") +
...
void BrowserApplication::loadSettings() {
  ...
  standardFont = settings.value(QLatin1String("standardFont"), standardFont)
                     .value<QFont>();
  defaultSettings->setFontFamily(QWebSettings::StandardFont,
                                 standardFont.family());
  defaultSettings->setFontSize(QWebSettings::DefaultFontSize,
                               standardFont.pointSize());
  int minimumFontSize =
      settings
          .value(QLatin1String("minimumFontSize"),
                 defaultSettings->fontSize(QWebSettings::MinimumFontSize))
          .toInt();
  ...
  fixedFont =
      settings.value(QLatin1String("fixedFont"), fixedFont).value<QFont>();
  defaultSettings->setFontFamily(QWebSettings::FixedFont, fixedFont.family());
  defaultSettings->setFontSize(QWebSettings::DefaultFixedFontSize,
                               fixedFont.pointSize());
  ...
  defaultSettings->setAttribute(
      ...
      settings.value(QLatin1String("enableInspector"), false).toBool());
  ...
  int maximumPagesInCache =
      settings.value(QLatin1String("maximumPagesInCache"), 3).toInt();
...
bool BrowserApplication::restoreLastSession() {
  {
    ...
    if (settings.value(QLatin1String("restoring"), false).toBool()) {
      QMessageBox::StandardButton result =
          QMessageBox::question(0, tr("Restore failed"),
                                tr("Arora crashed while trying to restore this "
                                   "session.  Should I try again?"),
...
BrowserMainWindow *BrowserApplication::newMainWindow() {
  ...
  connect(this, SIGNAL(privacyChanged(bool)), browser,
...
BrowserMainWindow *BrowserApplication::mainWindow() {
  ...
  if (m_mainWindows.isEmpty()) {
    ...
    activeWindow =
        qobject_cast<BrowserMainWindow *>(QApplication::activeWindow());
    if (!activeWindow)
...
LanguageManager *BrowserApplication::languageManager() {
  if (!s_languageManager) {
    ...
    s_languageManager->addLocaleDirectory(
        dataFilePath(QLatin1String("locale")));
    s_languageManager->addLocaleDirectory(qApp->applicationDirPath() +
                                          QLatin1String("/src/.qm/locale"));
    ...
    connect(s_languageManager, SIGNAL(languageChanged(const QString &)), qApp,
            SLOT(retranslate()));
  }
...
QIcon BrowserApplication::icon(const QUrl &url) {
  ...
  if (icon.isNull()) {
    QPixmap pixmap =
        QWebSettings::webGraphic(QWebSettings::DefaultFrameIconGraphic);
    if (pixmap.isNull()) {
      pixmap = QPixmap(QLatin1String(":graphics/defaulticon.png"));
      QWebSettings::setWebGraphic(QWebSettings::DefaultFrameIconGraphic,
                                  pixmap);
    }
    return pixmap;
  }
...
bool BrowserApplication::zoomTextOnly() {
  return QWebSettings::globalSettings()->testAttribute(
...
void BrowserApplication::setPrivate(bool isPrivate) {
  QWebSettings::globalSettings()->setAttribute(
      QWebSettings::PrivateBrowsingEnabled, isPrivate);
...
void BrowserApplication::setEventKeyboardModifiers(
