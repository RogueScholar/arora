...
ModelMenu::ModelMenu(QWidget *parent)
    : QMenu(parent), m_maxRows(-1), m_firstSeparator(-1), m_maxWidth(-1),
      m_statusBarTextRole(0), m_separatorRole(0), m_model(0) {
  setAcceptDrops(true);
...
bool ModelMenu::prePopulated() { return false; }

void ModelMenu::postPopulated() {}

void ModelMenu::setModel(QAbstractItemModel *model) { m_model = model; }

QAbstractItemModel *ModelMenu::model() const { return m_model; }

void ModelMenu::setMaxRows(int max) { m_maxRows = max; }

int ModelMenu::maxRows() const { return m_maxRows; }

void ModelMenu::setFirstSeparator(int offset) { m_firstSeparator = offset; }

int ModelMenu::firstSeparator() const { return m_firstSeparator; }

void ModelMenu::setRootIndex(const QModelIndex &index) { m_root = index; }

QModelIndex ModelMenu::rootIndex() const { return m_root; }

void ModelMenu::setStatusBarTextRole(int role) { m_statusBarTextRole = role; }

int ModelMenu::statusBarTextRole() const { return m_statusBarTextRole; }

void ModelMenu::setSeparatorRole(int role) { m_separatorRole = role; }

int ModelMenu::separatorRole() const { return m_separatorRole; }
...
ModelMenu *ModelMenu::createBaseMenu() { return new ModelMenu(this); }

void ModelMenu::createMenu(const QModelIndex &parent, int max,
  ...
  if (!menu) {
    ...
    disconnect(modelMenu, SIGNAL(triggered(QAction *)), modelMenu,
               SLOT(actionTriggered(QAction *)));
...
QAction *ModelMenu::makeAction(const QIcon &icon, const QString &text,
                               QObject *parent) {
...
void ModelMenu::dropEvent(QDropEvent *event) {
  ...
  event->acceptProposedAction();
  m_model->dropMimeData(event->mimeData(), event->dropAction(), row, 0,
                        parentIndex);
  QMenu::dropEvent(event);
}
