...
TabWidget::TabWidget(QWidget *parent)
    : QTabWidget(parent), m_recentlyClosedTabsAction(0), m_newTabAction(0),
      m_closeTabAction(0), m_bookmarkTabsAction(0), m_nextTabAction(0),
      m_previousTabAction(0), m_recentlyClosedTabsMenu(0),
      m_lineEditCompleter(0), m_locationBars(0), m_tabBar(new TabBar(this)),
      addTabButton(0), closeTabButton(0) {
  ...
  connect(m_tabBar, SIGNAL(loadUrl(const QUrl &, TabWidget::OpenUrlIn)), this,
          SLOT(loadUrl(const QUrl &, TabWidget::OpenUrlIn)));
  connect(m_tabBar, SIGNAL(newTab()), this, SLOT(newTab()));
  connect(m_tabBar, SIGNAL(closeTab(int)), this, SLOT(closeTab(int)));
  connect(m_tabBar, SIGNAL(cloneTab(int)), this, SLOT(cloneTab(int)));
  connect(m_tabBar, SIGNAL(closeOtherTabs(int)), this,
          SLOT(closeOtherTabs(int)));
  ...
  connect(m_bookmarkTabsAction, SIGNAL(triggered()), this,
          SLOT(bookmarkTabs()));
  ...
  connect(m_recentlyClosedTabsMenu, SIGNAL(aboutToShow()), this,
          SLOT(aboutToShowRecentTabsMenu()));
  connect(m_recentlyClosedTabsMenu, SIGNAL(triggered(QAction *)), this,
          SLOT(aboutToShowRecentTriggeredAction(QAction *)));
  ...
  connect(BrowserApplication::historyManager(), SIGNAL(historyCleared()), this,
          SLOT(historyCleared()));
...
void TabWidget::currentChanged(int index) {
  ...
  if (oldWebView) {
    disconnect(oldWebView, SIGNAL(statusBarMessage(const QString &)), this,
               SIGNAL(showStatusBarMessage(const QString &)));
    disconnect(
        oldWebView->page(),
        SIGNAL(linkHovered(const QString &, const QString &, const QString &)),
        this, SIGNAL(linkHovered(const QString &)));
    disconnect(oldWebView, SIGNAL(loadProgress(int)), this,
               SIGNAL(loadProgress(int)));
  }

  connect(webView, SIGNAL(statusBarMessage(const QString &)), this,
          SIGNAL(showStatusBarMessage(const QString &)));
  connect(
      webView->page(),
      SIGNAL(linkHovered(const QString &, const QString &, const QString &)),
...
QAction *TabWidget::newTabAction() const { return m_newTabAction; }

QAction *TabWidget::closeTabAction() const { return m_closeTabAction; }

QAction *TabWidget::bookmarkTabsAction() const { return m_bookmarkTabsAction; }

QAction *TabWidget::recentlyClosedTabsAction() const {
  return m_recentlyClosedTabsAction;
}

QAction *TabWidget::nextTabAction() const { return m_nextTabAction; }

QAction *TabWidget::previousTabAction() const { return m_previousTabAction; }

QWidget *TabWidget::locationBarStack() const { return m_locationBars; }
...
WebView *TabWidget::webView(int index) const {
  ...
  if (WebViewWithSearch *webViewWithSearch =
          qobject_cast<WebViewWithSearch *>(widget)) {
...
WebView *TabWidget::makeNewTab(bool makeCurrent) {
  ...
  if (!m_lineEditCompleter) {
    ...
    connect(m_lineEditCompleter, SIGNAL(activated(const QString &)), this,
            SLOT(loadString(const QString &)));
  ...
  connect(locationBar, SIGNAL(returnPressed()), this,
  ...
  QWidget::setTabOrder(
      locationBar,
      BrowserMainWindow::parentWindow(this)->findChild<ToolbarSearch *>());
#endif
  ...
  connect(webView, SIGNAL(titleChanged(const QString &)), this,
          SLOT(webViewTitleChanged(const QString &)));
  connect(webView, SIGNAL(urlChanged(const QUrl &)), this,
          SLOT(webViewUrlChanged(const QUrl &)));
  connect(webView, SIGNAL(search(const QUrl &, TabWidget::OpenUrlIn)), this,
          SLOT(loadUrl(const QUrl &, TabWidget::OpenUrlIn)));
  connect(webView->page(), SIGNAL(windowCloseRequested()), this,
          SLOT(windowCloseRequested()));
  connect(webView->page(), SIGNAL(printRequested(QWebFrame *)), this,
          SIGNAL(printRequested(QWebFrame *)));
  connect(webView->page(), SIGNAL(geometryChangeRequested(const QRect &)), this,
          SLOT(geometryChangeRequestedCheck(const QRect &)));
  connect(webView->page(), SIGNAL(menuBarVisibilityChangeRequested(bool)), this,
          SLOT(menuBarVisibilityChangeRequestedCheck(bool)));
  connect(webView->page(), SIGNAL(statusBarVisibilityChangeRequested(bool)),
          this, SLOT(statusBarVisibilityChangeRequestedCheck(bool)));
  connect(webView->page(), SIGNAL(toolBarVisibilityChangeRequested(bool)), this,
          SLOT(toolBarVisibilityChangeRequestedCheck(bool)));
...
QLabel *TabWidget::animationLabel(int index, bool addMovie) {
  ...
  QLabel *loadingAnimation =
      qobject_cast<QLabel *>(m_tabBar->tabButton(index, side));
  if (!loadingAnimation) {
    loadingAnimation = new QLabel(this);
  }
  if (addMovie && !loadingAnimation->movie()) {
    QMovie *movie = new QMovie(QLatin1String(":graphics/loading.gif"),
                               QByteArray(), loadingAnimation);
    movie->setSpeed(50);
...
void TabWidget::webViewLoadProgress(int progress) {
  ...
  QString message = tr("Loading %1% (%2 %3)...")
                        .arg(progress)
                        .arg(totalBytes, 0, 'f', 2)
                        .arg(QLatin1String("kB"));
...
#endif
    QIcon icon =
        BrowserApplication::instance()->icon(m_recentlyClosedTabs.at(i));
    action->setIcon(icon);
...
QUrl TabWidget::guessUrlFromString(const QString &string) {
  ...
  // We actually want to check if the url conforms to the RFC, which
  // QUrl::isValid() doesn't state.
  ...
  if (search) {
    url = ToolbarSearch::openSearchManager()->currentEngine()->searchUrl(
        string.trimmed());
  } else {
...
void TabWidget::loadSettings() {
  ...
  bool newTabButtonInRightCorner =
      settings.value(QLatin1String("newTabButtonInRightCorner"), true).toBool();
#ifndef Q_WS_MAC
  setCornerWidget(addTabButton, newTabButtonInRightCorner ? Qt::TopRightCorner
                                                          : Qt::TopLeftCorner);
  addTabButton->show();
#endif
  ...
  if (oneCloseButton) {
    ...
    setCornerWidget(closeTabButton, newTabButtonInRightCorner
                                        ? Qt::TopLeftCorner
                                        : Qt::TopRightCorner);
    closeTabButton->setVisible(oneCloseButton);
  } else {
    setCornerWidget(0, newTabButtonInRightCorner ? Qt::TopLeftCorner
                                                 : Qt::TopRightCorner);
  }
...
TabWidget::OpenUrlIn TabWidget::modifyWithUserBehavior(OpenUrlIn tab) {
  Qt::KeyboardModifiers modifiers =
      BrowserApplication::instance()->eventKeyboardModifiers();
  Qt::MouseButtons buttons =
      BrowserApplication::instance()->eventMouseButtons();
...
/*
   Somewhere in the browser interface a users wants to open a url in a specific
   tab.
 */
...
bool TabWidget::restoreState(const QByteArray &state) {
  ...
  for (int i = 0; i < openTabs.count(); ++i) {
    ...
    TabWidget::OpenUrlIn tab =
        i == 0 && currentWebView()->url() == QUrl() ? CurrentTab : NewTab;
#if QT_VERSION >= 0x040600
...
void TabWidget::createTab(const QByteArray &historyState,
                          TabWidget::OpenUrlIn tab) {
#if QT_VERSION >= 0x040600
  if (WebView *webView = getView(tab, currentWebView())) {
...
#else
  qWarning()
      << "Warning: TabWidget::createTab should not be called, but it is...";
