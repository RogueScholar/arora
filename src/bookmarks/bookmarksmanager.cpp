...
BookmarksManager::BookmarksManager(QObject *parent)
    : QObject(parent), m_loaded(false), m_saveTimer(new AutoSaver(this)),
      m_bookmarkRootNode(0), m_toolbar(0), m_menu(0), m_bookmarkModel(0) {
  connect(this, SIGNAL(entryAdded(BookmarkNode *)), m_saveTimer,
          SLOT(changeOccurred()));
  connect(this, SIGNAL(entryRemoved(BookmarkNode *, int, BookmarkNode *)),
          m_saveTimer, SLOT(changeOccurred()));
  connect(this, SIGNAL(entryChanged(BookmarkNode *)), m_saveTimer,
          SLOT(changeOccurred()));
}
...
void BookmarksManager::changeExpanded() { m_saveTimer->changeOccurred(); }
...
void BookmarksManager::load() {
  ...
  if (reader.error() != QXmlStreamReader::NoError) {
    QMessageBox::warning(
        0, QLatin1String("Loading Bookmark"),
  ...
  for (int i = m_bookmarkRootNode->children().count() - 1; i >= 0; --i) {
    ...
    if (node->type() == BookmarkNode::Folder) {
      ...
      if ((node->title == tr("Toolbar Bookmarks") ||
           node->title == QLatin1String(BOOKMARKBAR)) &&
          !m_toolbar) {
      ...
      if ((node->title == tr("Menu") ||
           node->title == QLatin1String(BOOKMARKMENU)) &&
          !m_menu) {
...
void BookmarksManager::addBookmark(BookmarkNode *parent, BookmarkNode *node,
                                   int row) {
  if (!m_loaded)
    return;
  Q_ASSERT(parent);
  InsertBookmarksCommand *command =
      new InsertBookmarksCommand(this, parent, node, row);
...
void BookmarksManager::removeBookmark(BookmarkNode *node) {
  ...
  RemoveBookmarksCommand *command =
      new RemoveBookmarksCommand(this, parent, row);
  m_commands.push(command);
}
...
void BookmarksManager::setTitle(BookmarkNode *node, const QString &newTitle) {
  ...
  Q_ASSERT(node);
  ChangeBookmarkCommand *command =
      new ChangeBookmarkCommand(this, node, newTitle, true);
  m_commands.push(command);
}
...
void BookmarksManager::setUrl(BookmarkNode *node, const QString &newUrl) {
  ...
  Q_ASSERT(node);
  ChangeBookmarkCommand *command =
      new ChangeBookmarkCommand(this, node, newUrl, false);
  m_commands.push(command);
}
...
void BookmarksManager::importBookmarks() {
  ...
  supportedFormats
      << tr("HTML Netscape bookmarks").append(QLatin1String("(*.html)"));

  QString fileName =
      QFileDialog::getOpenFileName(0, tr("Open File"), QString(),
  ...
  if (fileName.endsWith(QLatin1String(".html"))) {
    ...
    if (process.error() != QProcess::UnknownError) {
      if (process.error() == QProcess::FailedToStart) {
        QMessageBox::warning(
            0, tr("htmlToXBel tool required"),
            tr("htmlToXBel tool, which is shipped with Arora and is needed to "
...
RemoveBookmarksCommand::RemoveBookmarksCommand(
    BookmarksManager *m_bookmarkManagaer, BookmarkNode *parent, int row)
    : QUndoCommand(BookmarksManager::tr("Remove Bookmark")), m_row(row),
      m_bookmarkManagaer(m_bookmarkManagaer),
      m_node(parent->children().value(row)), m_parent(parent), m_done(false) {}
...
InsertBookmarksCommand::InsertBookmarksCommand(
    BookmarksManager *m_bookmarkManagaer, BookmarkNode *parent,
...
ChangeBookmarkCommand::ChangeBookmarkCommand(
    BookmarksManager *m_bookmarkManagaer, BookmarkNode *node,
    const QString &newValue, bool title)
    : QUndoCommand(), m_bookmarkManagaer(m_bookmarkManagaer), m_title(title),
      m_newValue(newValue), m_node(node) {
  if (m_title) {
