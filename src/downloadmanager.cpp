 ...
 */
DownloadItem::DownloadItem(QNetworkReply *reply, bool requestFileName,
                           QWidget *parent)
    : QWidget(parent), m_reply(reply), m_requestFileName(requestFileName),
      m_bytesReceived(0), m_startedSaving(false), m_finishedDownloading(false),
  ...
  if (!requestFileName) {
    ...
    m_requestFileName =
        settings.value(QLatin1String("alwaysPromptForFileName"), false)
            .toBool();
  }
...
void DownloadItem::getFileName() {
  ...
  QString downloadDirectory =
      BrowserApplication::downloadManager()->downloadDirectory();
  ...
  if (m_requestFileName) {
    ...
    if (fileName.isEmpty()) {
      ...
      stop();
      fileNameLabel->setText(tr("Download canceled: %1")
                                 .arg(QFileInfo(defaultFileName).fileName()));
      m_canceledFileSelect = true;
      return;
    }
    QFileInfo fileInfo = QFileInfo(fileName);
    BrowserApplication::downloadManager()->setDownloadDirectory(
  ...
  if (!saveDirPath.exists()) {
    if (!saveDirPath.mkpath(saveDirPath.absolutePath())) {
      ...
      downloadInfoLabel->setText(
          tr("Download directory (%1) couldn't be created.")
...
QString DownloadItem::saveFileName(const QString &directory) const {
  // Move this function into QNetworkReply to also get file name sent from the
  // server
  QString path;
  ...
  if (baseName.isEmpty()) {
    ...
    qDebug() << "DownloadItem::" << __FUNCTION__
             << "downloading unknown file:" << m_url;
#endif
  }
  ...
  if (!m_requestFileName && QFile::exists(name)) {
    ...
    int i = 1;
    do {
      name = directory + baseName + QLatin1Char('-') + QString::number(i++) +
             endName;
...
void DownloadItem::tryAgain() {
  ...
  QNetworkReply *r =
      BrowserApplication::networkAccessManager()->get(QNetworkRequest(m_url));
  if (m_reply)
    m_reply->deleteLater();
  if (m_output.exists())
    m_output.remove();
...
void DownloadItem::downloadReadyRead() {
  ...
  if (!m_output.isOpen()) {
    ...
    if (!m_output.open(QIODevice::WriteOnly)) {
      downloadInfoLabel->setText(
          tr("Error opening output file: %1").arg(m_output.errorString()));
      stop();
      emit statusChanged();
      return;
    }
  ...
  if (-1 == m_output.write(m_reply->readAll())) {
    downloadInfoLabel->setText(
        tr("Error saving: %1").arg(m_output.errorString()));
    stopButton->click();
  } else {
...
#ifdef DOWNLOADMANAGER_DEBUG
  qDebug() << "DownloadItem::" << __FUNCTION__ << m_reply->errorString()
           << m_url;
#endif

  downloadInfoLabel->setText(
      tr("Network Error: %1").arg(m_reply->errorString()));
  tryAgainButton->setEnabled(true);
...
double DownloadItem::remainingTime() const {
  ...
  double timeRemaining =
      ((double)(bytesTotal() - bytesReceived())) / currentSpeed();
...
bool DownloadItem::downloading() const { return (progressBar->isVisible()); }
...
/*!
    ...
    It is a basic download manager.  It only downloads the file, doesn't do
   BitTorrent, extract zipped files or anything fancy.
  */
DownloadManager::DownloadManager(QWidget *parent)
    : QDialog(parent), m_autoSaver(new AutoSaver(this)),
      m_model(new DownloadModel(this)),
      m_manager(BrowserApplication::networkAccessManager()), m_iconProvider(0),
...
bool DownloadManager::externalDownload(const QUrl &url) {
  ...
  QStringList args;
  for (int pos = 0; (pos = regex.indexIn(program, pos)) != -1;
       pos += regex.matchedLength())
    args << regex.cap(1) + regex.cap(2);
  if (args.isEmpty())
    return false;

  return QProcess::startDetached(args.takeFirst(),
...
#ifdef DOWNLOADMANAGER_DEBUG
  qDebug() << "DownloadManager::" << __FUNCTION__ << reply->url()
...
void DownloadManager::updateActiveItemCount() {
  ...
  if (acCount > 0) {
    setWindowTitle(
        QApplication::translate("DownloadDialog", "Downloading %1", 0)
            .arg(acCount));
  } else {
...
void DownloadManager::updateRow(DownloadItem *item) {
  ...
  downloadsView->setRowHeight(
      row, qMax(oldHeight, item->minimumSizeHint().height()));
...
void DownloadManager::save() const {
  ...
  for (int i = 0; i < m_downloads.count(); ++i) {
    ...
    settings.setValue(key + QLatin1String("done"),
                      m_downloads[i]->downloadedSuccessfully());
  }
...
void DownloadManager::load() {
  ...
  QMetaEnum removePolicyEnum = staticMetaObject.enumerator(
      staticMetaObject.indexOfEnumerator("RemovePolicy"));
  m_removePolicy =
      removePolicyEnum.keyToValue(value) == -1
          ? Never
  ...
  while (settings.contains(key + QLatin1String("url"))) {
    ...
    if (!url.isEmpty() && !fileName.isEmpty()) {
      ...
      item->fileNameLabel->setText(
          QFileInfo(item->m_output.fileName()).fileName());
      item->m_url = url;
...
QString DownloadManager::downloadDirectory() { return m_downloadDirectory; }
...
QVariant DownloadModel::data(const QModelIndex &index, int role) const {
  ...
  if (role == Qt::ToolTipRole)
    if (!m_downloadManager->m_downloads.at(index.row())
             ->downloadedSuccessfully())
      return m_downloadManager->m_downloads.at(index.row())
...
QMimeData *DownloadModel::mimeData(const QModelIndexList &indexes) const {
  ...
  foreach (const QModelIndex &index, indexes) {
    ...
    DownloadItem *item = m_downloadManager->m_downloads.at(index.row());
    urls.append(
        QUrl::fromLocalFile(QFileInfo(item->m_output).absoluteFilePath()));
  }
