...
int main(int argc, char **argv) {
  ...
  if (args.isEmpty()) {
    ...
    stream << "arora-cacheinfo is a tool for viewing and extracting "
              "information out of Arora cache files."
           << endl;
  ...
  if (!args.isEmpty() && args.count() >= 1 &&
    ...
    if (args.count() == 2) {
      ...
      if (fileName.isEmpty()) {
        qDebug() << "URL file name is empty, please specify an output file, I "
                    "wont guess.";
        return 1;
      }
      if (QFile::exists(fileName)) {
        qDebug() << "File already exists, not overwriting, please specify an "
                    "output file.";
        return 1;
      }
    }
  ...
  stream << "Last Modified Date: " << metaData.lastModified().toString()
         << endl;
  stream << "Save to disk: " << metaData.saveToDisk() << endl;
  stream << "Headers:" << endl;
  foreach (const QNetworkCacheMetaData::RawHeader &header,
           metaData.rawHeaders())
  ...
  if (device) {
    ...
    stream << "No data? Either the file is corrupt or there is an error."
           << endl;
  }
